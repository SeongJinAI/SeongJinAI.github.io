...
AI 에이전트에게 무제한 권한을 부여하는 것이 아닙니다. 에러가 감지될 때마다 **"이 에러에 대해서만, 이 범위에서만"** 제한된 권한을 부여합니다. 이것이 SOTT(Scoped One-Time Task) 패턴입니다.
...
```
                 ...
                 └─────────┬───────────┘
                           ▼
                    ┌──────────────┐
                    │  AI 자동 수정 │ ← 승인된 범위 내에서만 실행
                    │  브랜치 + PR  │
                    └──────┬───────┘
                           ▼
                    ┌──────────────┐
                    │  개발자 최종  │ ← 코드 확인 후 머지 결정
                    │  머지 결정   │
                    └──────────────┘
```
...
DB SQL 실행, 코드 머지는 **되돌리기 어려운 작업**입니다. AI가 99% 정확하더라도, 1%의 오판이 운영 데이터를 손상시킬 수 있습니다. 그래서:

- **분석/계획은 자동** — AI가 가장 잘하는 영역 (패턴 매칭, 코드 탐색)
- **실행 결정은 사람** — 되돌릴 수 없는 작업의 최종 게이트
- **반복 작업은 자동** — 브랜치 생성, 커밋, PR, 리뷰 반영 등 기계적 작업

### 개발자 개입 포인트 (3곳만)

| # | 지점 | SOTT 범위 | 소요 시간 |
|---|------|----------|----------|
| 1 | **수정 계획 승인** | "이 에러를 이 방법으로 수정해도 좋다" | 1분 |
| 2 | **DB SQL 실행** | "이 SQL을 운영 DB에 실행한다" | 1분 |
| 3 | **PR 최종 머지** | "이 코드를 dev에 반영한다" | 2분 |

나머지(감지, 분석, 유형 분류, 코드 수정, 브랜치 관리, PR 생성, 리뷰 반영)는 **전부 자동**입니다. 개발자는 **판단만 하고, 실행은 AI가** 합니다.
